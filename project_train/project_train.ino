...
int cmdTrain(int len, int paraNum)
{
  ...
  printSeperator();
  ret = myVR.train(records, paraNum-1, buf1);
...
int cmdLoad(int len, int paraNum)
{
  ...
  ret = myVR.load(records, paraNum-1, buf1);
  printSeperator();
  if(ret >= 0){
    printLoad(buf1, ret);
  }
...
int cmdRecord(int len, int paraNum)
{
  int ret;
  if(paraNum == 1){
    ret = myVR.checkRecord(buf1);
    printSeperator();
    if(ret>=0){
      printCheckRecordAll(buf1, ret);
    }
    else{
    ...
    ret = myVR.checkRecord(buf1, records, paraNum-1);    // auto clean duplicate records
    printSeperator();
    if(ret>=0){
      printCheckRecord(buf1, ret);
    }
    else{
      Serial.println(F("Check record failed or timeout."));
    }
...
int cmdSigTrain(int len, int paraNum)
{
  ...
  printSeperator();
  ret = myVR.trainWithSignature(records[0], paraAddr, sig_len, buf1);
  //  ret = myVR.trainWithSignature(records, paraNum-1);
  if(ret >= 0){
    printSigTrain(buf1, ret);
  }
  else{
...
int cmdGetSig(int len, int paraNum)
{
  ...
  ret = myVR.checkSignature(records[0], buf1);
...
int cmdSettings(int len, int paraNum)
{
  ...
  ret = myVR.checkSystemSettings(buf1);
  if( ret > 0){
    printSeperator();
    printSystemSettings(buf1, ret);
    printSeperator();
  }
  else{
    printSeperator();
